...
// Get BASE_URL from environment variable, fallback to localhost for development
...
`, (err) => {});
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_support_chats_user_phone ON support_chats(user_phone)`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_support_chats_status ON support_chats(status)`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_support_messages_chat_id ON support_messages(chat_id)`);
...
app.post("/api/support/chat", (req, res) => {
  ...
  // البحث عن شات مفتوح للعميل
  db.get(
    "SELECT * FROM support_chats WHERE user_phone = ? AND status = 'open' ORDER BY id DESC LIMIT 1",
    [user_phone],
    (err, chat) => {
      if (err) {
        return res.json({ success: false, message: "خطأ في قاعدة البيانات" });
      ...
      // إنشاء شات جديد
      db.run(
        "INSERT INTO support_chats (user_phone, user_name, user_email, status) VALUES (?, ?, ?, 'open')",
        [user_phone, user_name || null, user_email || null],
        function (err2) {
          if (err2) {
            return res.json({ success: false, message: "خطأ في إنشاء الشات" });
          }
          res.json({ success: true, chat: { id: this.lastID, user_phone, user_name, user_email, status: 'open' } });
        }
      );
    }
  );
...
app.get("/api/support/chat/:chatId/messages", (req, res) => {
  ...
  // التحقق من أن الشات يخص هذا العميل
  db.get("SELECT * FROM support_chats WHERE id = ? AND user_phone = ?", [chatId, user_phone], (err, chat) => {
    if (err || !chat) {
      return res.json({ success: false, message: "الشات غير موجود" });
    }
    ...
    db.all(
      ...
      (err2, messages) => {
        if (err2) {
          return res.json({ success: false, message: "خطأ في جلب الرسائل" });
        }

        // تحديث حالة القراءة للعميل
        db.run(
          "UPDATE support_messages SET read_by_user = 1 WHERE chat_id = ? AND sender_type = 'admin'",
          [chatId]
        );
...
// User: Send message
app.post("/api/support/chat/:chatId/messages", (req, res) => {
  const chatId = req.params.chatId;
  const { message, user_phone } = req.body;

  if (!message || !user_phone) {
    return res.json({ success: false, message: "الرسالة ورقم الهاتف مطلوبان" });
  }
  ...
  db.get("SELECT * FROM support_chats WHERE id = ? AND user_phone = ?", [chatId, user_phone], (err, chat) => {
    ...
    // التحقق من حالة الشات - إذا كان مغلقاً، نسمح بإرسال الرسالة لكن نعيد رسالة خاصة
    if (chat.status === 'closed') {
      // إرسال الرسالة مع إشعار خاص
      db.run(
        "INSERT INTO support_messages (chat_id, sender_type, message, read_by_admin) VALUES (?, 'user', ?, 0)",
        [chatId, message],
        function (err2) {
          if (err2) {
            return res.json({ success: false, message: "خطأ في إرسال الرسالة" });
          }

          // تحديث updated_at للشات
          db.run("UPDATE support_chats SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", [chatId]);

          // إعادة فتح الشات تلقائياً عند إرسال رسالة جديدة
          db.run("UPDATE support_chats SET status = 'open' WHERE id = ?", [chatId]);

          res.json({ 
            success: true, 
            messageId: this.lastID,
            chatReopened: true,
            notification: "تم إرسال رسالتك. سيتم الرد عليك بمجرد رؤية الرسالة."
          });
        }
      );
      return;
    ...
    // إذا كان الشات مفتوحاً، إرسال الرسالة بشكل عادي
    // التحقق من عدد الرسائل السابقة - إذا كانت هذه أول رسالة، نرسل إشعار
    db.get(
      "SELECT COUNT(*) as message_count FROM support_messages WHERE chat_id = ? AND sender_type = 'user'",
      [chatId],
      (err3, countData) => {
        if (err3) {
          return res.json({ success: false, message: "خطأ في التحقق من الرسائل" });
        }

        const isFirstMessage = countData.message_count === 0;
        ...
        db.run(
          ...
          function (err2) {
            ...
            // إذا كانت هذه أول رسالة، أرسل إشعار
            if (isFirstMessage) {
              res.json({ 
                success: true, 
                messageId: this.lastID,
                isFirstMessage: true,
                notification: "تم إرسال رسالتك. سيتم الرد عليك قريباً."
              });
            } else {
...
// Admin: Get all chats
app.get("/api/admin/support/chats", checkAdminAuth, (req, res) => {
  db.all(
    `SELECT 
      sc.*,
      (SELECT COUNT(*) FROM support_messages sm WHERE sm.chat_id = sc.id AND sm.sender_type = 'user' AND sm.read_by_admin = 0) as unread_count,
      (SELECT message FROM support_messages sm WHERE sm.chat_id = sc.id ORDER BY sm.created_at DESC LIMIT 1) as last_message,
      (SELECT created_at FROM support_messages sm WHERE sm.chat_id = sc.id ORDER BY sm.created_at DESC LIMIT 1) as last_message_time
    FROM support_chats sc
    ORDER BY sc.updated_at DESC`,
    [],
    (err, chats) => {
      if (err) {
        return res.json({ success: false, message: "خطأ في جلب الشاتات" });
      }
...
app.get("/api/admin/support/chat/:chatId/messages", checkAdminAuth, (req, res) => {
  const chatId = req.params.chatId;
  ...
  db.all(
    ...
    (err, messages) => {
      ...
      // تحديث حالة القراءة للإدارة
      db.run(
        "UPDATE support_messages SET read_by_admin = 1 WHERE chat_id = ? AND sender_type = 'user'",
        [chatId]
      );
...
// Admin: Send message
app.post("/api/admin/support/chat/:chatId/messages", checkAdminAuth, (req, res) => {
  const chatId = req.params.chatId;
  ...
  db.run(
    "INSERT INTO support_messages (chat_id, sender_type, message, read_by_user) VALUES (?, 'admin', ?, 0)",
    [chatId, message],
    function (err) {
      if (err) {
        return res.json({ success: false, message: "خطأ في إرسال الرسالة" });
...
// Admin: Close chat
app.put("/api/admin/support/chat/:chatId/close", checkAdminAuth, (req, res) => {
  const chatId = req.params.chatId;
  ...
  db.get("SELECT * FROM support_chats WHERE id = ?", [chatId], (err, chat) => {
    ...
    // إرسال رسالة تلقائية للعميل تخبره أن الشات تم إغلاقه
    const autoMessage = "تم إغلاق هذه المحادثة. إذا كان لديك أي استفسار آخر، يمكنك إرسال رسالة جديدة وسنرد عليك قريباً.";
    ...
    db.run(
      ...
      function (err2) {
        if (err2) {
          console.error("خطأ في إرسال الرسالة التلقائية:", err2);
        }

        // تحديث حالة الشات
        db.run(
          "UPDATE support_chats SET status = 'closed', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
          [chatId],
          function (err3) {
            if (err3) {
              return res.json({ success: false, message: "خطأ في إغلاق الشات" });
            }
            res.json({ success: true });
          }
        );
...
// Admin: Get unread chats count
app.get("/api/admin/support/unread-count", checkAdminAuth, (req, res) => {
  db.get(
...
// Admin: Get new chats count (chats with unread messages)
app.get("/api/admin/support/new-chats-count", checkAdminAuth, (req, res) => {
  db.get(
    `SELECT COUNT(DISTINCT sc.id) as count 
     FROM support_chats sc
     INNER JOIN support_messages sm ON sc.id = sm.chat_id
     WHERE sm.sender_type = 'user' AND sm.read_by_admin = 0`,
    [],
    (err, row) => {
      if (err) {
        return res.json({ success: false, count: 0 });
...
// Admin: Delete chat (only if closed)
app.delete("/api/admin/support/chat/:chatId", checkAdminAuth, (req, res) => {
  const chatId = req.params.chatId;

  // التحقق من أن الشات مغلق قبل الحذف
  db.get("SELECT * FROM support_chats WHERE id = ?", [chatId], (err, chat) => {
    if (err) {
      return res.json({ success: false, message: "خطأ في قاعدة البيانات" });
    }
    ...
    if (chat.status !== 'closed') {
      return res.json({ success: false, message: "يجب إغلاق الشات قبل حذفه" });
    }

    // حذف الرسائل أولاً (بسبب foreign key constraint)
    db.run("DELETE FROM support_messages WHERE chat_id = ?", [chatId], (err2) => {
      if (err2) {
        return res.json({ success: false, message: "خطأ في حذف الرسائل" });
      }

      // حذف الشات
      db.run("DELETE FROM support_chats WHERE id = ?", [chatId], (err3) => {
        if (err3) {
          return res.json({ success: false, message: "خطأ في حذف الشات" });
        }

        res.json({ success: true, message: "تم حذف الشات بنجاح" });
      });
    });
  });
});
...
// API endpoint to get BASE_URL (for frontend)
