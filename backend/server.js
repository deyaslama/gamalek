...
// Path configuration for Render.com persistence
...
// Configure multer to save files to persistent upload directory
const upload = multer({ 
  dest: uploadDir,
  limits: {
...
// SQLite database file in persistent data directory
...
}, express.static(path.join(rootDir, "admin")));

// صفحة layout.html - محمية
app.get("/layout.html", (req, res) => {
  if (req.session && req.session.adminLoggedIn === true) {
    return res.sendFile(path.join(rootDir, "layout.html"));
  }
  return res.redirect("/login.html");
});
...
// Serve other root files (must be last)
