...
function safeImageURL(url) {
  ...
  // If it's just a hash (like image filename without path), assume it's in /uploads/
  // Check if it looks like a hash/filename (no slashes, dots, or special chars except dashes/underscores)
...
// API URL is now set by config.js - use window.API or fallback to current origin
const API = window.API || (typeof window !== 'undefined' ? window.location.origin : "http://localhost:3000");
    ...
    grid.innerHTML += `
      <div class="brand-box" onclick="go('brand', ${b.id})">
        <div class="brand-circle">
          <img src="${safeImageURL(b.image_url)}">
        </div>
